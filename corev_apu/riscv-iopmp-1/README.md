...
Switch to `iopmp-stable` branch:

```bash
cd /work/<username>/riscv-iopmp/
git checkout iopmp-stable
```
...
- In the new window, click the Import Signals icon in the top left corner.
- Navigate to:

```
riscv-iopmp/verif/iopmp_top/
```
...
(This file contains all design signals grouped by module name.)
...
Every test listed in `Makefile_tests` starts with the model name it is designed for.  
For example, to run a test for the **Isolation Model**, uncomment any line that has `"isolation_model"` keyword in its test name.
