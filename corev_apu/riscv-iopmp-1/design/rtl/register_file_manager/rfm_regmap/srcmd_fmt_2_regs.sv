...
#(
  ...
  localparam MD_NUM_BY_8 = ((CFG.MD_NUM + 7)/8);
  ...
  generate
    ...
    for (genvar j = 0; j < MD_NUM_BY_8; j++) begin : gen_demux_enable_outer_loop
      ...
      for (genvar i = 0; i < 8; i++) begin : gen_demux_enable_inner_loop
        ...
        if ((j << 3) + i < CFG.MD_NUM) begin : valid_srcmd_index
  ...
  //****************************************************************************************************
  for (genvar srcmd_reg_index = 0; srcmd_reg_index < CFG.MD_NUM; srcmd_reg_index++) begin : gen_srcmd_regs_swdata

    // SRCMD_PERM(m) SW Write Data Signals
    assign srcmd_perm_swdata[srcmd_reg_index] = srcmd_perm_reg_swdata_q;

    // SRCMD_PERMH(m) SW Write Data Signals
    assign srcmd_permh_swdata[srcmd_reg_index] = srcmd_permh_reg_swdata_q;
  end
  ...
  always_comb begin : srcmd_fmt_2_write_path
    ...
    for (int j = 0; j < MD_NUM_BY_8; j++) begin : gen_write_outer_loop
      ...
      for (int i = 0; i < 8; i++) begin : gen_write_inner_loop
        ...
        if (srcmd_reg_index < CFG.MD_NUM) begin : gen_valid_srcmd_index_write
          ...
          if (CFG.IMP_MDLCK) begin : gen_mdlck_dependent_write
            ...
            // For MD 31 <= m <= 62, MDLCKH.mdh[m] locks both SRCMD_PERM(m) and SRCMD_PERMH(m)
              ...
              is_write_allowed = (!mdlckh_mdh[srcmd_reg_index-6'h1F]) && srcmd_reg_write_valid_q[j];
          end
  ...
  for (genvar curr_index = 0; curr_index < CFG.MD_NUM; curr_index++) begin : gen_srcmd_fmt_2_regs
  ...
  for (genvar curr_index = CFG.MD_NUM; curr_index < 63; curr_index++) begin : gen_srcmd_regs_hardwired_zeros
