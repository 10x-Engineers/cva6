...
#(
  ...
  if (CFG.MDCFG_FMT_2) begin : gen_md_entry_num_flop
    regfield #(
      .DW      (7),
      .SWACCESS("RW"),
      .RESVAL  ((CFG.MD_ENTRY_NUM <= MAX_MD_ENTRY_NUM_VALUE) ? CFG.MD_ENTRY_NUM : MAX_MD_ENTRY_NUM_VALUE)
    ...
    assign info_reg.hwcfg0.md_entry_num = (CFG.MD_ENTRY_NUM <= MAX_MD_ENTRY_NUM_VALUE) ?
                                          CFG.MD_ENTRY_NUM :
                                          MAX_MD_ENTRY_NUM_VALUE;
  end
  ...
  if (CFG.PRIENT_PROG) begin : gen_prio_entry_flop
    regfield #(
      .DW      (16),
      .SWACCESS("RW"),
      .RESVAL  ((CFG.PRIO_ENTRY <= MAX_SUPPORTED_PRIO_ENTRY) ? CFG.PRIO_ENTRY : MAX_SUPPORTED_PRIO_ENTRY)
    ) u_hwcfg2_prio_entry
    (
      .clk     (clk),
      .rst_n   (rst_n),
    ...
    assign info_reg.hwcfg2.prio_entry = (CFG.PRIO_ENTRY <= MAX_SUPPORTED_PRIO_ENTRY) ?
                                        CFG.PRIO_ENTRY :
                                        MAX_SUPPORTED_PRIO_ENTRY;
  end
