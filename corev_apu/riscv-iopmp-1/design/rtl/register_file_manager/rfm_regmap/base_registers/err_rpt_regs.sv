...
#(
  ...
  localparam logic [11:0] MAX_SVI_VALUE = (((CFG.RRID_NUM + 15)/16) - 1);   // Indicates the maximum error window index based on RRIDs
  ...
  if (CFG.ERROR_CAPTURE_EN) begin : gen_error_regs_write_path
    ...
    if (CFG.MFR_EN) begin : gen_err_mfr_swdata
      ...
      assign err_mfr_svi_swdata = (err_rpt_reg_swdata[27:16] <= MAX_SVI_VALUE) ?
                                  err_rpt_reg_swdata[27:16] :   // Write new data
                                  MAX_SVI_VALUE;                // Configure to maximum supported value
    end
