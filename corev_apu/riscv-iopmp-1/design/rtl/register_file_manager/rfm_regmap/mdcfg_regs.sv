...
#(
  ...
  input  logic [AHB_LITE_DATA_WIDTH-1:0] mdcfg_reg_swdata,        // Write data for MDCFG registers
  ...
  localparam NUM_MDCFG_DEMUXES = (CFG.MD_NUM + 7)/8;   // Indicates the number of MDCFG Register demuxes
  ...
  logic [AHB_LITE_DATA_WIDTH-1:0] mdcfg_reg_swdata_q;
  ...
  assign is_write_data_valid = (mdcfg_reg_swdata_q[15:0] <= hwcfg1_entry_num);
  ...
  //****************************************************************************************************
  for (genvar mdcfg_reg_index = 0; mdcfg_reg_index < CFG.MD_NUM; mdcfg_reg_index++) begin : gen_mdcfg_swdata
    ...
    assign mdcfg_t_swdata[mdcfg_reg_index] = is_write_data_valid ?
                                             mdcfg_reg_swdata_q[15:0] :       // Write new data
  ...
  always_comb begin : mdcfg_write_path
    ...
    for (int j = 0; j < NUM_MDCFG_DEMUXES; j++) begin : gen_write_outer_loop
      ...
      for (int i = 0; i < 8; i++) begin : gen_write_inner_loop
        ...
        if (mdcfg_reg_index < CFG.MD_NUM) begin : gen_valid_mdcfg_write
          ...
          if ((mdcfg_reg_demux_sel_q == i) && mdcfg_demux_enable[j]) begin
  ...
  for (genvar curr_index = 0; curr_index < CFG.MD_NUM; curr_index++) begin : gen_mdcfg_regs
  ...
  for (genvar curr_index = CFG.MD_NUM; curr_index < 63; curr_index++) begin : gen_mdcfg_regs_hardwired_zeros
