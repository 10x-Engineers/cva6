...
#(
  ...
  localparam RRID_NUM_BY_8 = ((CFG.RRID_NUM + 7)/8);
  ...
  logic [AHB_LITE_DATA_WIDTH-1:0] srcmd_en_reg_swdata_q, srcmd_enh_reg_swdata_q, srcmd_r_reg_swdata_q, srcmd_rh_reg_swdata_q, srcmd_w_reg_swdata_q, srcmd_wh_reg_swdata_q;
  int                             srcmd_reg_index;
  ...
  generate
    ...
    for (genvar j = 0; j < RRID_NUM_BY_8; j++) begin : gen_demux_enable_outer_loop
      ...
      for (genvar i = 0; i < 8; i++) begin : gen_demux_enable_inner_loop
        ...
        if (((j << 3) + i) < CFG.RRID_NUM) begin : valid_srcmd_index
  ...
  //****************************************************************************************************
  for (genvar srcmd_reg_index = 0; srcmd_reg_index < CFG.RRID_NUM; srcmd_reg_index++) begin : gen_srcmd_regs_swdata

    assign srcmd_en_l_swdata[srcmd_reg_index] = srcmd_en_reg_swdata_q[0];
    ...
    if (CFG.IMP_MDLCK) begin : gen_mdlck_dependent_swdata
      ...
      assign srcmd_en_md_swdata[srcmd_reg_index] = (((srcmd_en_reg_swdata_q[31:1] & ~mdlck_md) | (srcmd_table_0[srcmd_reg_index].srcmd_en.md & mdlck_md)) & MD_NUM_MASK[30:0]);
      ...
      assign srcmd_enh_swdata[srcmd_reg_index]   = (((srcmd_enh_reg_swdata_q & ~mdlckh_mdh) | (srcmd_table_0[srcmd_reg_index].srcmd_enh.mdh & mdlckh_mdh)) & MD_NUM_MASK[62:31]);
      ...
      if (CFG.SPS_EN) begin : gen_sps_regs_swdata_0
        ...
        assign srcmd_r_swdata[srcmd_reg_index]  = (((srcmd_r_reg_swdata_q[31:1] & ~mdlck_md) | (srcmd_table_0[srcmd_reg_index].srcmd_r.md & mdlck_md)) & MD_NUM_MASK[30:0]);
        ...
        assign srcmd_rh_swdata[srcmd_reg_index] = (((srcmd_rh_reg_swdata_q & ~mdlckh_mdh) | (srcmd_table_0[srcmd_reg_index].srcmd_rh.mdh & mdlckh_mdh)) & MD_NUM_MASK[62:31]);
        ...
        assign srcmd_w_swdata[srcmd_reg_index]  = (((srcmd_w_reg_swdata_q[31:1] & ~mdlck_md) | (srcmd_table_0[srcmd_reg_index].srcmd_w.md & mdlck_md)) & MD_NUM_MASK[30:0]);
        ...
        assign srcmd_wh_swdata[srcmd_reg_index] = (((srcmd_wh_reg_swdata_q & ~mdlckh_mdh) | (srcmd_table_0[srcmd_reg_index].srcmd_wh.mdh & mdlckh_mdh)) & MD_NUM_MASK[62:31]);
      end
    end
    ...
    // Write on SRCMD Table registers/fields is independent of MDLCK or MDLCKH if IMP_MDLCK is 0
      ...
      assign srcmd_en_md_swdata[srcmd_reg_index] = srcmd_en_reg_swdata_q[31:1] & MD_NUM_MASK[30:0];

      // The fields SRCMD_ENH(s).md[m] are not writable if MD m is not supported
      assign srcmd_enh_swdata[srcmd_reg_index]   = srcmd_enh_reg_swdata_q & MD_NUM_MASK[62:31];

      if (CFG.SPS_EN) begin : gen_sps_regs_swdata_1

        // The fields SRCMD_R(s).md[m] are not writable if MD m is not supported
        assign srcmd_r_swdata[srcmd_reg_index]  = srcmd_r_reg_swdata_q & MD_NUM_MASK[30:0];

        // The fields SRCMD_RH(s).md[m] are not writable if MD m is not supported
        assign srcmd_rh_swdata[srcmd_reg_index] = srcmd_rh_reg_swdata_q & MD_NUM_MASK[62:31];

        // The fields SRCMD_W(s).md[m] are not writable if MD m is not supported
        assign srcmd_w_swdata[srcmd_reg_index]  = srcmd_w_reg_swdata_q & MD_NUM_MASK[30:0];

        // The fields SRCMD_WH(s).md[m] are not writable if MD m is not supported
        assign srcmd_wh_swdata[srcmd_reg_index] = srcmd_wh_reg_swdata_q & MD_NUM_MASK[62:31];
      end
    end
  end
  ...
  always_comb begin : srcmd_fmt_0_write_path
    ...
    for (int j = 0; j < RRID_NUM_BY_8; j++) begin : gen_write_outer_loop
      ...
      for (int i = 0; i < 8; i++) begin : gen_write_inner_loop
        ...
        if (srcmd_reg_index < CFG.RRID_NUM) begin : gen_valid_srcmd_index_write
          ...
          if (srcmd_demux_enable[srcmd_reg_index]) begin : gen_write_demux_enable
            ...
            if (CFG.SPS_EN) begin : sps_regs_write_0
          ...
          // Drive SW write enable signals zero when demux is not enabled
            ...
            if (CFG.SPS_EN) begin : sps_regs_write_1
              srcmd_r_swen[srcmd_reg_index]  = 1'b0;
              srcmd_rh_swen[srcmd_reg_index] = 1'b0;
              srcmd_w_swen[srcmd_reg_index]  = 1'b0;
              srcmd_wh_swen[srcmd_reg_index] = 1'b0;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (CFG.SPS_EN) begin : sps_regs_write_data_1
        srcmd_r_reg_swdata_q  <= srcmd_reg_swdata;
        srcmd_rh_reg_swdata_q <= srcmd_reg_swdata;
        srcmd_w_reg_swdata_q  <= srcmd_reg_swdata;
  ...
  for (genvar curr_index = 0; curr_index < CFG.RRID_NUM; curr_index++) begin : gen_srcmd_fmt_0_regs
  ...
  // Generate SRCMD registers based on RRID_NUM value when SRCMD Format is 0
  for (genvar curr_index = CFG.RRID_NUM; curr_index < 64; curr_index++) begin : gen_srcmd_regs_hardwired_zeros
