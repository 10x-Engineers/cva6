...
#(
  ...
  localparam ENTRY_NUM_BY_16  = (CFG.ENTRY_NUM + 15)/16;
  ...
  logic [AHB_LITE_DATA_WIDTH-1:0]     entry_addr_0_swdata_q, entry_addr_1_swdata_q, entry_addrh_0_swdata_q, entry_addrh_1_swdata_q, entry_cfg_0_swdata_q, entry_cfg_1_swdata_q;
  ...
  generate
    ...
    for (genvar j = 0; j < ENTRY_NUM_BY_16; j++) begin : gen_demux_enable_outer_loop
      ...
      for (genvar i = 0; i < 8; i++) begin : gen_demux_enable_inner_loop
        ...
        if (((i << 1) + (j << 4)) < CFG.ENTRY_NUM) begin : valid_entry
  ...
  always_comb begin : entry_array_write_path
    ...
    for (int j = 0; j < ENTRY_NUM_BY_16; j++) begin : gen_write_outer_loop
      ...
      for (int i = 0; i < 8; i++) begin : gen_write_inner_loop
        ...
        if (entry_array_reg_index < CFG.ENTRY_NUM) begin : gen_valid_entry_regs_write
          ...
          is_entry_array_reg_locked = (({4'b0000, entry_array_reg_demux_sel_q[j]} >> 2) + ({4'b0000, entry_array_selected_demux_q[j][2:0]} << 1) + ({4'b0000, entry_array_selected_demux_q[j][5:3]} << 4)) < entrylck_f;
          ...
          if (entry_array_demux_enable[((j << 3) + i)]) begin : write_demux_enable
            ...
            unique case (entry_array_reg_demux_sel_q[j])
              ...
              IOPMP_ENTRY_ADDR_1: if ((entry_array_reg_index + 1) < CFG.ENTRY_NUM) begin : gen_valid_entry_addr_write

                // ENTRY_ADDR(i) is locked for i < entrylck.f.
                entry_addr_swen[(entry_array_reg_index + 1)] = is_write_allowed;
              end

              IOPMP_ENTRY_ADDRH_1: if (((entry_array_reg_index + 1) < CFG.ENTRY_NUM) && CFG.ADDRH_EN) begin : gen_entry_addrh_1_write

                // ENTRY_ADDRH(i) is locked for i < entrylck.f.
                entry_addrh_swen[(entry_array_reg_index + 1)] = is_write_allowed;
              end

              IOPMP_ENTRY_CFG_1: if ((entry_array_reg_index + 1) < CFG.ENTRY_NUM) begin : gen_valid_entry_cfg_write
  ...
  always_comb begin : gen_napot_size_valid_range_vect
    ...
    for (int entry_array_reg_index = 0; entry_array_reg_index < CFG.ENTRY_NUM; entry_array_reg_index++) begin : gen_entry_address
      curr_entry_address |= {(AXI_ADDR_WIDTH-2){napot_size_swen_q[entry_array_reg_index]}} &
                            {entry_array[entry_array_reg_index].entry_addrh.addrh, entry_array[entry_array_reg_index].entry_addr.addr};

      // Previous entry address will be determined if the address mode TOR is supported
      if (CFG.TOR_EN) begin : gen_prev_entry_addr
        prev_entry_address |= (entry_array_reg_index == 0) ? '0 : {(AXI_ADDR_WIDTH-2){napot_size_swen_q[entry_array_reg_index]}} &
                                                                  {entry_array[entry_array_reg_index-1].entry_addrh.addrh, entry_array[entry_array_reg_index-1].entry_addr.addr};
      end
    end
  ...
  //****************************************************************************************************
  for (genvar entry_array_reg_index = 0; entry_array_reg_index < ((CFG.ENTRY_NUM + 1)/2); entry_array_reg_index++) begin : gen_entry_array_swdata_0

    // ENTRY_ADDR(i) SW Write Data Signals
    assign entry_addr_swdata[entry_array_reg_index] = entry_addr_0_swdata_q;

    // ENTRY_ADDRH(i) SW Write Data Signals
    assign entry_addrh_swdata[entry_array_reg_index] = entry_addrh_0_swdata_q[17:0];

    // ENTRY_CFG(i) SW Write Data Signals
    assign entry_cfg_r_swdata[entry_array_reg_index] = entry_cfg_0_swdata_q[0];
    assign entry_cfg_w_swdata[entry_array_reg_index] = entry_cfg_0_swdata_q[1];
    assign entry_cfg_x_swdata[entry_array_reg_index] = entry_cfg_0_swdata_q[2];

    if (CFG.TOR_EN) begin
      assign entry_cfg_a_swdata[entry_array_reg_index] = entry_cfg_0_swdata_q[4:3];
    end
    else begin
      assign entry_cfg_a_swdata[entry_array_reg_index] = ((entry_cfg_0_swdata_q[4:3] != rfm_pkg::IOPMP_TOR) ?   // When TOR_EN is 0, the data 2'b01 becomes illegal
                                                         entry_cfg_0_swdata_q[4:3] :                            // Write new data
                                                         entry_array[entry_array_reg_index].entry_cfg.a);       // Retain the old data when new data is 2'b01
    end

    assign entry_cfg_sire_swdata[entry_array_reg_index] = entry_cfg_0_swdata_q[5];
    assign entry_cfg_siwe_swdata[entry_array_reg_index] = entry_cfg_0_swdata_q[6];
    assign entry_cfg_sixe_swdata[entry_array_reg_index] = entry_cfg_0_swdata_q[7];

    assign napot_size_swdata[entry_array_reg_index] = trail_zeros;
  end

  for (genvar entry_array_reg_index = ((CFG.ENTRY_NUM + 1)/2); entry_array_reg_index < CFG.ENTRY_NUM; entry_array_reg_index++) begin : gen_entry_array_swdata_1

    // ENTRY_ADDR(i) SW Write Data Signals
    assign entry_addr_swdata[entry_array_reg_index] = entry_addr_1_swdata_q;

    // ENTRY_ADDRH(i) SW Write Data Signals
    assign entry_addrh_swdata[entry_array_reg_index] = entry_addrh_1_swdata_q[17:0];

    // ENTRY_CFG(i) SW Write Data Signals
    assign entry_cfg_r_swdata[entry_array_reg_index] = entry_cfg_1_swdata_q[0];
    assign entry_cfg_w_swdata[entry_array_reg_index] = entry_cfg_1_swdata_q[1];
    assign entry_cfg_x_swdata[entry_array_reg_index] = entry_cfg_1_swdata_q[2];

    if (CFG.TOR_EN) begin
      assign entry_cfg_a_swdata[entry_array_reg_index] = entry_cfg_1_swdata_q[4:3];
    end
    else begin
      assign entry_cfg_a_swdata[entry_array_reg_index] = ((entry_cfg_1_swdata_q[4:3] != rfm_pkg::IOPMP_TOR) ?   // When TOR_EN is 0, the data 2'b01 becomes illegal
                                                         entry_cfg_1_swdata_q[4:3] :                            // Write new data
                                                         entry_array[entry_array_reg_index].entry_cfg.a);       // Retain the old data when new data is 2'b01
    end

    assign entry_cfg_sire_swdata[entry_array_reg_index] = entry_cfg_1_swdata_q[5];
    assign entry_cfg_siwe_swdata[entry_array_reg_index] = entry_cfg_1_swdata_q[6];
    assign entry_cfg_sixe_swdata[entry_array_reg_index] = entry_cfg_1_swdata_q[7];
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      entry_addrh_0_swdata_q        <= entry_array_reg_swdata;
      entry_addrh_1_swdata_q        <= entry_array_reg_swdata;
      entry_cfg_0_swdata_q          <= entry_array_reg_swdata;
      entry_cfg_1_swdata_q          <= entry_array_reg_swdata;
  ...
  generate
    ...
    for (genvar curr_index = 0; curr_index < CFG.ENTRY_NUM; curr_index++) begin : gen_entry_array_reg
    ...
    for (genvar curr_index = CFG.ENTRY_NUM; curr_index < 128; curr_index++) begin : gen_entry_array_regs_hardwired_zeros
