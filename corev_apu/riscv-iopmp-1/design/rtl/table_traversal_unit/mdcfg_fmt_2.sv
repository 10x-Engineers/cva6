...
/// is 2, this module generates upto 3 pipeline stages where each stage
...
#(
  ...
  // Axi Master Request Manager ==> MDCFG Format 2
  input  transaction_t                    transaction_i,        // AXI transaction

  // TTU Checks ==> MDCFG Format 2
  ...
  // Register File Manager ==> MDCFG Format 2
  input  logic [2:0]                      hwcfg0_md_entry_num,  // hwcfg0.md_entry_num + 1 indicates the number of entries associated with an MD when MDCFG Format is 2
  input  srcmd_table_2_t [CFG.MD_NUM-1:0] srcmd_table_2,        // SRCMD Table contains SRCMD_PERM(H) in SRCMD Format 2

  // SRCMD Table Traversal ==> MDCFG Format 2
  ...
  // MDCFG Format 2 ==> Rule Analyzer Pipeline
  ...
  // Intermediate stages flops when MDCFG Format is 2
  ...
  logic         [NUM_TTU_STAGES-1:0][3:0]                    k_l_i;                     // Number of entries associated with an MD at the input of each pipeline stage
  logic         [NUM_TTU_STAGES-1:0]                         is_k_power_of_2_l_i;       // Indicates if k (hwcfg0_md_entry_num+1) is a power of 2 at the input of each pipeline stage
  logic         [NUM_TTU_STAGES-1:0][1:0]                    clog2_k_l_i;               // Indicates if log2(k) when k is power of 2 at the input of each pipeline stage
  ...
  logic         [NUM_TTU_STAGES-1:0][3:0]                    k_l_o;                     // Number of entries associated with an MD at the output of each pipeline stage
  logic         [NUM_TTU_STAGES-1:0]                         is_k_power_of_2_l_o;       // Indicates if k (hwcfg0_md_entry_num+1) is a power of 2 at the output of each pipeline stage
  logic         [NUM_TTU_STAGES-1:0][1:0]                    clog2_k_l_o;               // Indicates if log2(k) when k is power of 2 at the output of each pipeline stage
  ...
  logic         [AXI_ADDR_WIDTH-1:0]                         end_addr;                  // Transaction end address
  ...
  assign k_l_i[0] = {1'b0,hwcfg0_md_entry_num} + 4'h1;     // k_l0 at the input of slice 1 indicates the number of entries associated with an MD

  // This line checks if k is a power of 2 as power of 2 number has only one bit set, AND with hwcfg0_md_entry_num clears all the bits,
  // so k & hwcfg0_md_entry_num will be zero only if k is a powers of 2
  // The reduction OR '|' collapses all bits of the result to 1 if any bit is set, else 0
  // Then the outer '!' negates it: true (1) if result is 0, meaning k is a power of 2
  assign is_k_power_of_2_l_i[0] = (!(|(k_l_i[0] & hwcfg0_md_entry_num)));   // is_k_power_of_2_l0 at the input of slice 1 indicates if k is a power of 2 or not

  always_comb begin

    // Find the log2 of k if it is a power of 2, then we can only use a shifter to get i*k (or (i+1)*k)
    case (k_l_i[0])
      4'b0010: clog2_k_l_i[0] = 2'b01;
      4'b0100: clog2_k_l_i[0] = 2'b10;
      4'b1000: clog2_k_l_i[0] = 2'b11;
      default: clog2_k_l_i[0] = 2'b00;
    endcase
  end

  // The loop generates a number of stages pipeline where each stage processes a distinct chunk of MDs
  // using the mds_traversal_fmt_2 module. Signals are forwarded from one stage to the next at every clock cycle
  ...
  for (genvar curr_stage = 0; curr_stage < NUM_TTU_STAGES; curr_stage++) begin : gen_mds_traversal_units
    ...
    localparam int END_MD   = ((curr_stage + 1) * 21 > CFG.MD_NUM) ? CFG.MD_NUM : ((curr_stage + 1) * 21);
    ...
    (
      // MDCFG Format 2 ==> MDs Traversal Format 2
      ...
      .k_l_i               (k_l_i[curr_stage]),                      // k_l_i[0] input to slice 1, k_l_i[1] input to slice 2, k_l_i[2] input to slice 3
      .is_k_power_of_2_l_i (is_k_power_of_2_l_i[curr_stage]),        // is_k_power_of_2_l_i[0] input to slice 1, is_k_power_of_2_l_i[1] input to slice 2, is_k_power_of_2_l_i[2] input to slice 3
      .clog2_k_l_i         (clog2_k_l_i[curr_stage]),                // clog2_k_l_i[0] input to slice 1, clog2_k_l_i[1] input to slice 2, clog2_k_l_i[2] input to slice 3
      ...
      // MDs Traversal Format 2 ==> MDCFG Format 2
      ...
      .k_l_o               (k_l_o[curr_stage]),                      // k_l_o[0] output of slice 1, k_l_o[1] output of slice 2, k_l_o[2] output of slice 3
      .is_k_power_of_2_l_o (is_k_power_of_2_l_o[curr_stage]),        // is_k_power_of_2_l_o[0] output of slice 1, is_k_power_of_2_l_o[1] output of slice 2, is_k_power_of_2_l_o[2] output of slice 3
      .clog2_k_l_o         (clog2_k_l_o[curr_stage]),                // clog2_k_l_o[0] outputt of slice 1, clog2_k_l_o[1] output of slice 2, clog2_k_l_o[2] output of slice 3
  ...
  for (genvar curr_stage = 1; curr_stage < NUM_TTU_STAGES; curr_stage++) begin : gen_intermediate_flops
    always_ff @(posedge clk or negedge rst_n) begin
      if (!rst_n) begin
        ...
        k_l_i[curr_stage]                 <= '0;
        is_k_power_of_2_l_i[curr_stage]   <= '0;
        clog2_k_l_i[curr_stage]           <= '0;
        ...
        k_l_i[curr_stage]                 <= k_l_o[curr_stage-1];
        is_k_power_of_2_l_i[curr_stage]   <= is_k_power_of_2_l_o[curr_stage-1];
        clog2_k_l_i[curr_stage]           <= clog2_k_l_o[curr_stage-1];
  ...
  assign end_addr = (({1'b0, transaction_l[NUM_TTU_STAGES-1].addr[11:0]} + (((transaction_l[NUM_TTU_STAGES-1].len & {8{(|transaction_l[NUM_TTU_STAGES-1].burst)}}) + {{7{1'b0}},1'b1}) << transaction_l[NUM_TTU_STAGES-1].size)) - {{12{1'b0}},1'b1});
  ...
  assign final_ttu_rap_operation = (updated_rap_operation[NUM_TTU_STAGES-1][1] && (!updated_rap_operation[NUM_TTU_STAGES-1][0]) && end_addr[12]) ?
                                   ERROR : updated_rap_operation[NUM_TTU_STAGES-1];

  assign final_ttu_rap_err_info  = (updated_rap_operation[NUM_TTU_STAGES-1][1] && (!updated_rap_operation[NUM_TTU_STAGES-1][0]) && end_addr[12]) ?
