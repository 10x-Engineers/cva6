...
/// Date Created: 12-June-2025
/// Description: This module implements the MDCFG Table Traversal. When
/// MDCFG Format is 0, this module generates upto 3 pipeline stages where
/// each stage can prcoess upto 21 memory domains. The supported number
/// of MDs dictates the pipeline stages and the number of cycles required
/// to traverse the MDs before the final output is sent to Rule Analyzer
/// Pipeline for further address checks and permissions matches.
///////////////////////////////////////////////////////////////////////////
...
#(
  ...
  // Axi Master Request Manager ==> MDCFG Format 0
  input  transaction_t                    transaction_i,       // AXI transaction

  // TTU Checks ==> MDCFG Format 0
  ...
  // Register File Manager ==> MDCFG Format 0
  input  mdcfg_t [CFG.MD_NUM-1:0]         mdcfg_table,         // MDCFG Table required when MDCFG Format is 0
  input  srcmd_table_2_t [CFG.MD_NUM-1:0] srcmd_table_2,       // SRCMD Table contains SRCMD_PERM(H) in SRCMD Format 2

  // SRCMD Table Traversal ==> MDCFG Format 0
  ...
  // MDCFG Format 0 ==> Rule Analyzer Pipeline
  ...
  // Intermediate stages flops when MDCFG Format is 0
  ...
  logic         [NUM_TTU_STAGES-1:0][CFG.ENTRY_NUM-1:0]      bv_or_l_i;                 // Indicates the OR of bitmap vector of all the previous MDs at the input of each pipeline stage
  ...
  logic         [NUM_TTU_STAGES-1:0][CFG.ENTRY_NUM-1:0]      bv_or_l_o;                 // Indicates the OR of bitmap vector of all the previous MDs at the output of each pipeline stage
  ...
  assign bv_or_l_i[0]             = '0;               // bv_or_l0 at the input of slice 1
  ...
  // using the mdcfg_table_traversal module Signals are forwarded from one stage to the next at every clock cycle
  ...
  for (genvar curr_stage = 0; curr_stage < NUM_TTU_STAGES; curr_stage++) begin : gen_mds_traversal_units
    ...
    (
      ...
      .transaction_rrid_l (transaction_l[curr_stage].rrid[4:0]),    // transaction_l[0].rrid input to slice 1, transaction_l[1].rrid input to slice 2, transaction_l[2].rrid input to slice 3
      ...
      .bv_or_l_i          (bv_or_l_i[curr_stage]),                  // bv_or_l_i[0] input to slice 1, bv_or_l_i[1] input to slice 2, bv_or_l_i[2] input to slice 3
      ...
      .mdcfg_table        (mdcfg_table[END_MD-1:START_MD]),         // MDCFG Table. Each stage inputs the required MDCFG Registers
      .srcmd_table        (srcmd_table_2[END_MD-1:START_MD]),       // SRCMD Table for extratcting RW permissions when SRCMD Format 2. Each stage inputs the required SRCMD Table Registers
      ...
      .bv_or_l_o          (bv_or_l_o[curr_stage]),                  // bv_or_l_o[0] output of slice 1, bv_or_l_o[1] output of slice 2, bv_or_l_o[2] output of slice 3
    ...
    //****************************************************************************************************
    // MDCFG Table Traversal Stages Flops
    //****************************************************************************************************
  ...
  for (genvar curr_stage = 1; curr_stage < NUM_TTU_STAGES; curr_stage++) begin : gen_inter_flops
    always_ff @(posedge clk or negedge rst_n) begin
      if (!rst_n) begin
        ...
        bv_or_l_i[curr_stage]             <= bv_or_l_o[curr_stage-1];
