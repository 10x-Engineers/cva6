...
#include  "../../iopmp_top/vlib/dpi_defines.h"
...
#else
    #include "../../iopmp_ref_model/verif/test_utils.h"
#endif
#ifndef UVM_TEST
    #include "iopmp.h"
#else
    ...
    #include "config.h"
#else
    #include "../../iopmp_ref_model/include/config.h"
#endif
...
uint8_t is_access_valid(uint64_t offset, uint8_t num_bytes) {
    ...
    if ((num_bytes != 4 && num_bytes != 8) || (num_bytes > REG_INTF_BUS_WIDTH) ||
        ...
        ((offset & (num_bytes - 1)) != 0)) {
            // vpi_printf("\n check 1:::::::: %d", (num_bytes != 4 && num_bytes != 8));
            // vpi_printf("\n check 2:::::::: %d", (num_bytes > REG_INTF_BUS_WIDTH));
            // vpi_printf("\n check 3:::::::: %d", (offset >= (0x1014 + (IOPMP_RRID_NUM * 32)) && !iopmpRule_range) );
            // vpi_printf("\n check 4:::::::: %d", ((offset & (num_bytes - 1)) != 0));
        ...
        if (ENTRY_TABLE_INDEX(offset) >= g_reg_file.entrylck.f) {
            switch (entry_reg)
            {
                ...
                case 0:
                    ...
                    // vpi_printf("\n I have a write on entry_addr %x\n", entry_addr_temp.raw );
                ...
                case 1:
                    ...
                    // vpi_printf("\n I have a write on entry_addrh %x\n", entry_addrh_temp.raw );
                    #if (IOPMP_ADDRH_EN)
