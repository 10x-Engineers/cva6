  ...
  rap_cover_error_response: cover property (
    @(posedge clk) ((!`RAP_RESP_GEN.rap_stage.operation[0]) |-> (`RAP_RESP_GEN.is_resp_error))
  );
  ...
  rap_cover_rd_ex_error_resp: cover property (
    @(posedge clk) (((`RAP_RESP_GEN.rap_stage.transaction.r || `RAP_RESP_GEN.rap_stage.transaction.x) && (!`RAP_RESP_GEN.rap_stage.operation[0])) |-> (`RAP_RESP_GEN.rap_rd_err_valid))
  );
  ...
  rap_cover_wr_error_resp: cover property (
    @(posedge clk) ((`RAP_RESP_GEN.rap_stage.transaction.w && (!`RAP_RESP_GEN.rap_stage.operation[0])) |-> (`RAP_RESP_GEN.rap_wr_err_valid))
  );
  ...
  rap_cover_success_response: cover property (
    @(posedge clk) ((`RAP_RESP_GEN.rap_stage.operation == MATCHED) |-> (`RAP_RESP_GEN.is_resp_success))
  );
  ...
  rap_cover_rd_ex_success_resp: cover property (
    @(posedge clk) (((`RAP_RESP_GEN.rap_stage.transaction.r || `RAP_RESP_GEN.rap_stage.transaction.x) && (`RAP_RESP_GEN.rap_stage.operation == MATCHED)) |-> (`RAP_RESP_GEN.rap_rd_valid))
  );
  ...
  rap_cover_wr_success_resp: cover property (
    @(posedge clk) ((`RAP_RESP_GEN.rap_stage.transaction.w && (`RAP_RESP_GEN.rap_stage.operation == MATCHED)) |-> (`RAP_RESP_GEN.rap_wr_valid))
  );
  ...
  if (config_iopmp_pkg::iopmp_cfg_default.ERROR_CAPTURE_EN) begin : error_report_enabled
    ...
    rap_cover_report_error_to_eic: cover property (
      @(posedge clk) ((!`RAP_RESP_GEN.rap_stage.operation[0]) |-> (`RAP_RESP_GEN.rap_eic_valid))
    );
    ...
    rap_cover_reported_error_type: cover property (
      @(posedge clk) (((`RAP_RESP_GEN.rap_stage.operation == SEARCH) && (!(|`RAP_RESP_GEN.rap_stage.err_info))) |-> (`RAP_RESP_GEN.rap_eic_error_info.err_info == {NOT_HIT_ANY_RULE, 1'b0}))
    );
  ...
  // ####################################### ERRROR REPORT ENABLED #####################################
    ...
    rap_assert_error_type: assert property (
      @(posedge clk) disable iff (~tb_top.iopmp_dut.rst_n) ((`RAP_RESP_GEN.rap_stage.operation == ERROR) |-> (|`RAP_RESP_GEN.rap_stage.err_info))
  ...
  // ###################################### ERRROR REPORT DISABLED #####################################
    ...
    rap_assert_error_report_disable: assert property (
      @(posedge clk) disable iff (~tb_top.iopmp_dut.rst_n) ((!`RAP_RESP_GEN.rap_stage.operation[0]) |-> (`RAP_RESP_GEN.rap_eic_valid != 1'b1))
    ) else $error("[%0t] Assertion Failed: Partial Adress matched hit for a Non-Priority Entry", $time);
  end