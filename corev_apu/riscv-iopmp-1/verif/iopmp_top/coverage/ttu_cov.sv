  ...
  ttu_assert_no_write_access: assert property (
    @(posedge clk) disable iff (~tb_top.iopmp_dut.rst_n) (((`TTU.rap_operation == SEARCH) && `TTU.transaction_i.w && `TTU.rfm_ttu.hwcfg0_enable) |-> ((|`TTU.ttu_operation) != 1'b1))
  ...
  ttu_assert_no_instr_fetch_access: assert property (
    @(posedge clk) disable iff (~tb_top.iopmp_dut.rst_n) (((`TTU.rap_operation == SEARCH) && `TTU.transaction_i.x && `TTU.rfm_ttu.hwcfg0_enable) |-> ((|`TTU.ttu_operation) != 1'b1))
  ...
  ttu_assert_iopmp_disable: assert property (
    @(posedge clk) disable iff (~tb_top.iopmp_dut.rst_n) (((`TTU.rap_operation == SEARCH) && (!`TTU.rfm_ttu.hwcfg0_enable)) |-> (`TTU.ttu_operation[0] != 1'b0))
  ...
  ttu_assert_axi_req_no_search: assert property (
    @(posedge clk) disable iff (~tb_top.iopmp_dut.rst_n) ((`TTU.rap_operation != SEARCH) |-> ((&`TTU.ttu_operation) != 1'b0))
  ...
  // TTU.21. Cover Property: If any transaction crosses 4KB boundary, then end_addr[12] bit should be 1
  ttu_cover_4kb_check: cover property (
    @(posedge clk) (((`LAST_STAGE.transaction_l[`TTU.NUM_TTU_STAGES-1].size == 3'b011) && (`LAST_STAGE.transaction_l[`TTU.NUM_TTU_STAGES-1].len == 8'h0F) && (|`LAST_STAGE.transaction_l[`TTU.NUM_TTU_STAGES-1].burst)
    && ((&`LAST_STAGE.transaction_l[`TTU.NUM_TTU_STAGES-1].addr[11:7]) && (|`LAST_STAGE.transaction_l[`TTU.NUM_TTU_STAGES-1].addr[6:0]))) |-> (`LAST_STAGE.end_addr[12]))
  );

  // TTU.22. Cover Property: If any transaction crosses 4KB boundary, then TTU operation should be error if the operation from last stage is SEARCH
  ...
  // TTU.23. Cover Property: If any transaction crosses 4KB boundary, then TTU should indicates NOT_HIT_ANY_RULE error
  ttu_cover_4kb_check_fail_error: cover property (
    @(posedge clk) (((`LAST_STAGE.updated_rap_operation[`TTU.NUM_TTU_STAGES-1] == SEARCH) && `LAST_STAGE.end_addr[12]) |=> (`LAST_STAGE.ttu_rap_err_info == NOT_HIT_ANY_RULE))
  );