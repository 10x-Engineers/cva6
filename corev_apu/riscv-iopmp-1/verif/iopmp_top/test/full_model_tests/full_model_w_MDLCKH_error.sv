class full_model_w_MDLCKH_error extends full_model_base_test;
  `uvm_component_utils(full_model_w_MDLCKH_error)
    mdlckh_reg_write         mdlckh;
    srcmdenh_reg_write        srcmdenh;
    srcmdrh_reg_write         srcmdrh;
    srcmdwh_reg_write         srcmdwh;
    mdcfg_reg_write          mdcfg;
    entry_addr_reg_write     entry_addr;
    entry_cfg_reg_write      entry_cfg;
    axi_req_seq_w_napot      aw_seq;
    axi_req_seq_w_data_napot aw_seq_data;
    //  iopmp_env     env;

  function new(string name = "full_model_w_MDLCKH_error", uvm_component parent = null);
     super.new(name, parent);
      mdlckh      = mdlckh_reg_write::type_id::create("mdlckh");
      srcmdenh     = srcmdenh_reg_write::type_id::create("srcmdenh");
      srcmdrh      = srcmdrh_reg_write::type_id::create("srcmdrh");
      srcmdwh      = srcmdwh_reg_write::type_id::create("srcmdwh");
      mdcfg       = mdcfg_reg_write::type_id::create("mdcfg");
      entry_addr  = entry_addr_reg_write::type_id::create("entry_addr");
      entry_cfg   = entry_cfg_reg_write::type_id::create("entry_cfg");
      aw_seq      = axi_req_seq_w_napot::type_id::create("aw_seq");
      aw_seq_data = axi_req_seq_w_data_napot::type_id::create("aw_seq_data");
   endfunction

  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      phase.raise_objection(this, "MAIN - raise_objection");

      mdlckh.wdata = 64'h100000000;
      mdlckh.start(env.ahb_env.ahb_agnt.ahb_sqr);

      srcmdenh.wdata = 64'h100000000;
      srcmdenh.index = 32;
      srcmdenh.start(env.ahb_env.ahb_agnt.ahb_sqr);

      srcmdrh.wdata = 64'h100000000;
      srcmdrh.index = 32;
      srcmdrh.start(env.ahb_env.ahb_agnt.ahb_sqr);

      srcmdwh.wdata = 32'h10;
      srcmdwh.index = 32;
      srcmdwh.start(env.ahb_env.ahb_agnt.ahb_sqr);

      mdcfg.wdata = 64'h500000000;                  // 5 entries associated with this md   entry 0,1,2,3,4
      mdcfg.index = 31;
      mdcfg.start(env.ahb_env.ahb_agnt.ahb_sqr);

      // hwcfg2.prio_entry = 16
      entry_addr.wdata = 32'd90;
      entry_addr.index = 3;
      entry_addr.start(env.ahb_env.ahb_agnt.ahb_sqr);

      entry_cfg.wdata = 32'h1B;                     // NAPOT address mode with read and write permission granted
      entry_cfg.index = 3;
      entry_cfg.start(env.ahb_env.ahb_agnt.ahb_sqr);

      // This transaction will be an error as no entry will be searched in RAP
      #50ns;
      aw_seq.rrid = 32;
      aw_seq.addr = 360;
      aw_seq.id   = 3;
      fork
          aw_seq.start(env.axi_env.wr_addr_agnt.wr_addr_sqr);              //write access test
          aw_seq_data.start(env.axi_env.wr_data_agnt.wr_data_sqr);
      join_any
      #700;
      phase.drop_objection(this, "MAIN - drop_objection");
    `uvm_info(get_name(), "MAIN PHASE ENDED", UVM_LOW);
  endtask : main_phase
endclass