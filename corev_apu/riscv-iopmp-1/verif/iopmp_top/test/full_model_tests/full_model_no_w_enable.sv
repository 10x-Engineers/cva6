class full_model_no_w_enable extends full_model_base_test;
  `uvm_component_utils(full_model_no_w_enable)
  ...
  function new(string name = "full_model_no_w_enable", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      entry_cfg.wdata = 32'hB;                      // TOR address mode with write permission granted and read permission not granted
      entry_cfg.index = 1;
      entry_cfg.start(env.ahb_env.ahb_agnt.ahb_sqr);

      // NOT HIT ANY RULE Error on this transaction as NO_W is set
      #50ns;
      aw_seq.rrid = 2;
      aw_seq.addr = 364;
      aw_seq.id   = 3;
