/*************************************************************************
   > File Name:   full_model_NA4_illegal_read_mfr_en_svw_1_rrid_test.sv
   ...
   2. Send a read./write/instruction fetch transaction that must generate some error.
   3. Send the same transaction again to generate the subsequent violation.
   4. Send a read request on ERR_MFR.
   5. Send the same transaction again with different RRID that must belong to the same error window as the above transaction.
   6. The transaction error must be logged in the error window at the same time when the read request on the ERR_MFR clears the window.
   7. Send a read request on ERR_MFR again to check the error window.
   ...
   ---------------------------------------------------------------
************************************************************************/
class full_model_NA4_illegal_read_mfr_en_svw_1_rrid_test extends full_model_base_test;
  `uvm_component_utils(full_model_NA4_illegal_read_mfr_en_svw_1_rrid_test)
  ...
  function new(string name = "full_model_NA4_illegal_read_mfr_en_svw_1_rrid_test", uvm_component parent = null);
