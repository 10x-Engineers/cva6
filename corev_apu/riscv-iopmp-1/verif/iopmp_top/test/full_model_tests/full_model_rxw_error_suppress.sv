class full_model_rxw_error_suppress extends full_model_base_test;
  `uvm_component_utils(full_model_rxw_error_suppress)
  ...
  function new(string name = "full_model_rxw_error_suppress", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      entry_cfg.wdata = 32'h710;                     // NA4 address mode with ewe, ere, exe suppression and no permissions granted
      entry_cfg.index = 5;
      entry_cfg.start(env.ahb_env.ahb_agnt.ahb_sqr);

      // The transaction is a success but error is reported to EIC Block and logged
      #50ns;
      ar_seq.rrid      = 32;
      ar_seq.addr      = 364;
      ar_seq.id        = 3;
      ...
      // The transaction is a success but error is reported to EIC Block but not logged
      #50ns;
      ar_seq.rrid      = 32;
      ar_seq.addr      = 364;
      ar_seq.id        = 3;
