/*************************************************************************
   > File Name:   full_model_NA4_illegal_instruction_fetch_interrupt_enable_err_sup.sv
   > Description: Send instruction fetch for Full MOdel with NA4 Addressing mode with no permission in entry_cfg. Enable interrupt enable bit in error capture register then check if interrupt is reported or not. Set sexe bit
   ...
   ---------------------------------------------------------------
*************************************************************************/
class full_model_NA4_illegal_instruction_fetch_interrupt_enable_err_sup extends full_model_base_test;
  `uvm_component_utils(full_model_NA4_illegal_instruction_fetch_interrupt_enable_err_sup)
  ...
  function new(string name = "full_model_NA4_illegal_instruction_fetch_interrupt_enable_err_sup", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
     ...
     // hwcfg2.prio_entry = 16
      ...
      entry_cfg.b_data   = 32'h410;                     // NA4 address mode with no permissions granted and sexe bit enabled
      entry_cfg.index    = 3;
