/*************************************************************************
   > File Name:   fm_NA4_rd_non_pri_partial_hit_int_en.sv
   > Description: Send read address for Full Model with NA4 Addressing mode with read permission in entry_cfg. Set size of data such that it gives partial hit. Also Enable interrupt enable bit in error capture register then check if interrupt is reported or not. Set entry non priority
   ...
   ---------------------------------------------------------------
************************************************************************/
class fm_NA4_rd_non_pri_partial_hit_int_en extends full_model_base_test;
  `uvm_component_utils(fm_NA4_rd_non_pri_partial_hit_int_en)
  ...
  function new(string name = "fm_NA4_rd_non_pri_partial_hit_int_en", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      mdcfg.b_data       = 32'h31;                          // 5 entries associated with this md   entry 0...48
      mdcfg.index        = 55;
      mdcfg.start(env.reg_env.ahb_agnt.ahb_sqr);
      
      mdcfg.b_data       = 32'h32;                          // 5 entries associated with this md   entry 49
      mdcfg.index        = 56;
      mdcfg.start(env.reg_env.ahb_agnt.ahb_sqr);
