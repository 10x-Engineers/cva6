/*************************************************************************
   > File Name:   full_model_x_perm_read_no_x_chk_x.sv
   > Description: Send read address with prot b100 for Full MOdel with NA4 Addressing mode by passing rx permission and enabling no_x  and chk_x in HWCFG in entry_cfg.
   >              Set CHK_X -> 1 || NO_X -> 1
   >              Set     IOPMP_CHK_X -> 1 ||     IOPMP_NO_X -> 1
   ...
   ---------------------------------------------------------------
************************************************************************/
class full_model_x_perm_read_no_x_chk_x extends full_model_base_test;
  `uvm_component_utils(full_model_x_perm_read_no_x_chk_x)
  ...
  function new(string name = "full_model_x_perm_read_no_x_chk_x", uvm_component parent = null);
  ...
  super.main_phase(phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      //-------------------------------------------------
      //                            Assign 
      //                   CHK_X           = 1;  Also chnage parameter in reference model folder file config.h
      //                   NO_X            = 1;
      //                     Then run test
      //-------------------------------------------------
      ...
      entry_cfg.b_data = 32'h15;                     // NA4 address mode with no permission granted
      entry_cfg.index = 3;
