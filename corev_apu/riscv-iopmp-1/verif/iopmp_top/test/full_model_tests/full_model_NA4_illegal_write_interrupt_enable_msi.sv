/*************************************************************************
   > File Name:   full_model_NA4_illegal_write_interrupt_enable_msi.sv
   > Description: Send write address and data for Full MOdel with NA4 Addressing mode with no permission in entry_cfg. Enable interrupt enable and msi_en bit in error capture register then check if interrupt is reported or not.
...
class full_model_NA4_illegal_write_interrupt_enable_msi extends full_model_base_test;
  `uvm_component_utils(full_model_NA4_illegal_write_interrupt_enable_msi)
  ...
  function new(string name = "full_model_NA4_illegal_write_interrupt_enable_msi", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      err_cfg.b_data     = 32'h4010A;                      // interrupt disable
