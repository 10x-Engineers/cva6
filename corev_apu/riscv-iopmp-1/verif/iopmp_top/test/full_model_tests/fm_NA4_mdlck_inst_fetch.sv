/*************************************************************************
   > File Name:   fm_NA4_mdlck_inst_fetch.sv
   > Description: Send instruction fetch for Full MOdel with NA4 Addressing mode with rx permission in entry_cfg. Initially don't set the MDLCK. Set f=4 in MDLCK after setting srcmden = 8 and mdcfg 5. After this try to set srcmden = 4. Then set mdlck = E. 
   ...
   ---------------------------------------------------------------
************************************************************************/
class fm_NA4_mdlck_inst_fetch extends full_model_base_test;
    `uvm_component_utils(fm_NA4_mdlck_inst_fetch)
    ...
    function new(string name = "fm_NA4_mdlck_inst_fetch", uvm_component parent = null);
    ...
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
        ...
        srcmden.b_data = 32'hC;                        // Traverse MD 1
        srcmden.index = 5;
        srcmden.start(env.reg_env.ahb_agnt.ahb_sqr);

        srcmdr.b_data = 32'hC;                          // Traverse MD 1
        srcmdr.index = 5;
        srcmdr.start(env.reg_env.ahb_agnt.ahb_sqr);
