/*************************************************************************
   > File Name:   fm_NA4_PP_mdstallh_ex.sv
   > Description: Send read address for Full Model with NA4 Addressing mode with no permission in entry_cfg.  Select MDs BY SETTING md field in MDSTALLh reg. Also set exempt to exclude those mds from stalling. Then send read transaction again.
   ...
   ---------------------------------------------------------------
************************************************************************/
class fm_NA4_PP_mdstallh_ex extends full_model_base_test;
  `uvm_component_utils(fm_NA4_PP_mdstallh_ex)
    ...
    mdstall_reg_seq       mdstall_ex;

    axi_req_seq           ar_seq;
    axi_s_sanity_seq      s_seq;

  function new(string name = "fm_NA4_PP_mdstallh_ex", uvm_component parent = null);
     super.new(name, parent);
      ...
      mdstall_ex    = mdstall_reg_seq::type_id::create("mdstall");
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      srcmden.b_data = 32'h8;                        // Traverse MD 34
      srcmden.index = 5;
     ...
     // hwcfg2.prio_entry = 16
      ...
      mdstall_ex.b_data    = 32'h1;
      mdstall_ex.start(env.axi_env.rd_addr_agnt.rd_addr_sqr);
