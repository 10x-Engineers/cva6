/*************************************************************************
   > File Name:   full_model_NA4_src_rw_no_perm_write.sv
   > Description: Send write address for Full MOdel with NA4 Addressing mode with no permission in entry_cfg. Set corresponding bits in SRCMD R and W.
   1. Configure full address match and entry permissions with read=0 and write=0.
   2. Corresponding SRCMD_R=1 and SRCMD_W=1.
...
class full_model_NA4_src_rw_no_perm_write extends full_model_base_test;
  `uvm_component_utils(full_model_NA4_src_rw_no_perm_write)
  ...
  function new(string name = "full_model_NA4_src_rw_no_perm_write", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      entry_cfg.b_data = 32'h10;                     // NA4 address mode with read and write permission granted
      entry_cfg.index = 5;
