/*************************************************************************
   > File Name:   full_model_NA4_illegal_read_msi_interrupt_enable_int_dis.sv
   > Description: 
    1. Set MSI_EN flag in the IOPMP Configurations.
    2. Program IOPMP Registers and disable global interrupt (ie) bit.
    3. Send any read/write/eX transaction to the IOPMP that should generate an error.
   ...
   ---------------------------------------------------------------
************************************************************************/
class full_model_NA4_illegal_read_msi_interrupt_enable_int_dis extends full_model_base_test;
  `uvm_component_utils(full_model_NA4_illegal_read_msi_interrupt_enable_int_dis)
  ...
  function new(string name = "full_model_NA4_illegal_read_msi_interrupt_enable_int_dis", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
     ...
     // hwcfg2.prio_entry = 16
      ...
      entry_cfg.b_data   = 32'h10;                     // NA4 address mode with no permissions granted and sire enabled.
      entry_cfg.index    = 3;
      entry_cfg.start(env.reg_env.ahb_agnt.ahb_sqr);

      err_cfg.b_data     = 32'h40108;                      // interrupt enable
