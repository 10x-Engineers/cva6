/*************************************************************************
   > File Name:   full_model_NA4_illegal_write_interrupt_enable_err_sup.sv
   > Description: Send write address and data for Full MOdel with NA4 Addressing mode with no permission in entry_cfg. Enable interrupt enable bit in error capture register then check if interrupt is reported or not. Set sewe bit
...
class full_model_NA4_illegal_write_interrupt_enable_err_sup extends full_model_base_test;
  `uvm_component_utils(full_model_NA4_illegal_write_interrupt_enable_err_sup)
  ...
  function new(string name = "full_model_NA4_illegal_write_interrupt_enable_err_sup", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      entry_cfg.b_data = 32'h210;                     // NA4 address mode with no permission granted and enable sewe bit.
      entry_cfg.index = 5;
