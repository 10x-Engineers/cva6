/*************************************************************************
   > File Name:   fm_NA4_mdcfglck_read.sv
   > Description: Send read address for Full MOdel with NA4 Addressing mode with r permission in entry_cfg. Set f=4 in MDCFGLCK after setting MD to 2 by writting srcmden = 8 and setting mdcfg = 5. After this try to set mdcfg = 3. Then try to set f in MDCFGLCK smaller than the previous one. 
   ...
   ---------------------------------------------------------------
************************************************************************/
class fm_NA4_mdcfglck_read extends full_model_base_test;
    `uvm_component_utils(fm_NA4_mdcfglck_read)
    ...
    function new(string name = "fm_NA4_mdcfglck_read", uvm_component parent = null);
