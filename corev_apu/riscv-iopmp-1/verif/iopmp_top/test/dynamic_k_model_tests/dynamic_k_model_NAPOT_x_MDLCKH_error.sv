...
class dynamic_k_model_NAPOT_x_MDLCKH_error extends dynamic_k_model_base_test;
  `uvm_component_utils(dynamic_k_model_NAPOT_x_MDLCKH_error)
  ...
  function new(string name = "dynamic_k_model_NAPOT_x_MDLCKH_error", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      // This transaction will be an error as no entry is searched in RAP
      #50ns;
      ar_seq.rrid      = 2;
      ar_seq.addr      = 360;
      ar_seq.id        = 3;
