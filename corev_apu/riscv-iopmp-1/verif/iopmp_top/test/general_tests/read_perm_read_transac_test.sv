...
class TEST_NAME extends base_test;
  ...
  task main_phase(uvm_phase phase);
    ...
    mdcfg_improper = CFG_MDCFG_IMP;       //high this Parameter to initialize improper mdcfgs
    r_perm         = CFG_R_PERM;          //high this bit for read perm
    w_perm         = CFG_W_PERM;          //high this bit for write perm
    x_perm         = CFG_X_PERM;          //high this bit for instruction fetch perm
    ad_mode        = CFG_AD_MODE;         //OFF=0, TOR=1, NA4=2, NAPOT=3
    t_type         = CFG_TN;              //transaction type 0= read, 1 = write, 2 = instruction fetch
    intr_en        = CFG_INTR_EN;         //Error capture field
    err_sup        = CFG_ERR_SUP;         //Error capture field
    msi_en         = CFG_MSI_EN;          //Error capture field
    stall_voi_en   = CFG_STALL_VOI_EN;    //Error capture field    
    msi_data       = CFG_MSI_DATA;        //Error capture field 
