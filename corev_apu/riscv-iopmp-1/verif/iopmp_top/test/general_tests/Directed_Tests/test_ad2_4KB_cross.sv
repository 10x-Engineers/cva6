/*************************************************************************
   > File Name:   test_ad2_4KB_cross.sv
   > Description: 
   ...
   ---------------------------------------------------------------
************************************************************************/
class test_ad2_4KB_cross extends base_test;
  `uvm_component_utils(test_ad2_4KB_cross)
  ...
  function new(string name = "test_ad2_4KB_cross", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    ...
    `endif
      ...
      if(t_type==0)begin
        ...
        aw_seq.seq_addr   = 32'hFFF<<1;
        end
        aw_seq.seq_id     = seq_id;
        aw_seq.burst_t    = INCR;
        aw_seq.seq_size   = transac_size;
        aw_seq.data_size  = data_size;     //Assign Data size to be written
        aw_seq.length     = 0;    //pass burst length manually
        aw_seq.has_length = 0;    //pass flag to sequence  (0 to not)
        //W data channel
        aw_seq_data.seq_addr   = 32'hFFF<<1;
