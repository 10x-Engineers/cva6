/*************************************************************************
   > File Name:   full_model_x0_perm_read_chk_x_disable.sv
   > Description: Send read address with prot b100 for Full MOdel with NA4 Addressing mode by passing no read permission and disabling chk_x in HWCFG ->(x permission will become don't care) in entry_cfg.
   >              This an illegal condition because chk_x imploys read permission must be 1.
   >              Set CHK_X -> 0
   >              Set     IOPMP_CHK_X -> 0
   1. Configure full address match and entry with execute=1.
   2. Corresponding SRCMD_R=1.
   3. HWCFG0.chk_x=0.
   ...
   ---------------------------------------------------------------
************************************************************************/
class full_model_x0_perm_read_chk_x_disable extends full_model_base_test;
  `uvm_component_utils(full_model_x0_perm_read_chk_x_disable)
  ...
  function new(string name = "full_model_x0_perm_read_chk_x_disable", uvm_component parent = null);
