/*************************************************************************
   > File Name:   full_model_NA4_illegal_read_mfr_en_svi_write_test.sv
   > Description: Enable MFR. Write smaller pssoible value for window index then equal to largest value and then also larger than the largest possible value.
   ...
   ---------------------------------------------------------------
************************************************************************/
class full_model_NA4_illegal_read_mfr_en_svi_write_test extends full_model_base_test;
  `uvm_component_utils(full_model_NA4_illegal_read_mfr_en_svi_write_test)
  ...
  function new(string name = "full_model_NA4_illegal_read_mfr_en_svi_write_test", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      err_mfr.b_data= 32'h30000;
      err_mfr.start(env.reg_env.ahb_agnt.ahb_sqr);

      err_mfr.b_data= 32'h40000;
      err_mfr.start(env.reg_env.ahb_agnt.ahb_sqr);
