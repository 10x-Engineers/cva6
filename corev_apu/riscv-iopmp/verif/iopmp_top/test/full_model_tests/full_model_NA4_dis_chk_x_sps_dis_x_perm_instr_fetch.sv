/*************************************************************************
   > File Name:   full_model_NA4_dis_chk_x_sps_dis_x_perm_instr_fetch.sv
   > Description: Send read address with prot b100 for Full MOdel with NA4 Addressing mode by passing intruction fetch permission and disabling chk_x in HWCFG->(x permission will become don't care) in entry_cfg. Will generate error because read perm is disabled.
   >              Set CHK_X -> 0      
   >              Set      OPMP_CHK_X -> 0 
   ...
   3. Configure the associated entry with execute permission only.
   ...
   ---------------------------------------------------------------
************************************************************************/
class full_model_NA4_dis_chk_x_sps_dis_x_perm_instr_fetch extends full_model_base_test;
  `uvm_component_utils(full_model_NA4_dis_chk_x_sps_dis_x_perm_instr_fetch)
  ...
  function new(string name = "full_model_NA4_dis_chk_x_sps_dis_x_perm_instr_fetch", uvm_component parent = null);
  ...
  super.main_phase(phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      entry_cfg.b_data = 32'h14;                     // NA4 address mode with read and execute permission granted
      entry_cfg.index = 3;
