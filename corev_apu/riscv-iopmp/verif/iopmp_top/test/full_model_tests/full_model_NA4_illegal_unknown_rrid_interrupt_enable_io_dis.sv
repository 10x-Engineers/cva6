/*************************************************************************
   > File Name:   full_model_NA4_illegal_unknown_rrid_interrupt_enable_io_dis.sv
   > Description: Send read address for Full MOdel with NA4 Addressing mode with read permission in entry_cfg. Also Enable interrupt enable bit in error capture register then check if interrupt is reported or not. Send request with unknown rrid.  Disable IOPMP enable hwcfg0.enable=0.
   ...
   ---------------------------------------------------------------
************************************************************************/
class full_model_NA4_illegal_unknown_rrid_interrupt_enable_io_dis extends full_model_base_test;
  `uvm_component_utils(full_model_NA4_illegal_unknown_rrid_interrupt_enable_io_dis)
  ...
  function new(string name = "full_model_NA4_illegal_unknown_rrid_interrupt_enable_io_dis", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
     ...
     // hwcfg2.prio_entry = 16
      ...
      hwcfg0.b_data = 32'h0000;                   //Setting IOPMP Enable 1
