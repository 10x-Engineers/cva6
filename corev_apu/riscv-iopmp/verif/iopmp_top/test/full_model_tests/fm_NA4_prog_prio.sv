/*************************************************************************
   > File Name:   fm_NA4_prog_prio.sv
   > Description: Send read address for Full Model with NA4 Addressing mode with r permission in entry_cfg. Set prio_entry in HWCFG2 as prient_prog in HWCFG0 is enabled through parameters in default confg.
   >              Set SE_EN -> 1
   >              Set SRC_ENFORCEMENT_EN -> 1
   ...
   ---------------------------------------------------------------
************************************************************************/
class fm_NA4_prog_prio extends full_model_base_test;
  `uvm_component_utils(fm_NA4_prog_prio)
  ...
  function new(string name = "fm_NA4_prog_prio", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      hwcfg2.b_data = 32'hA;                        // Program PRIO
      ...
      hwcfg2.b_data = 32'h31;                        // Program PRIO
      hwcfg2.start(env.reg_env.ahb_agnt.ahb_sqr);
