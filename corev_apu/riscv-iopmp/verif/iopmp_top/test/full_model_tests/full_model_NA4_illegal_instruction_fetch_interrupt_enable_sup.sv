/*************************************************************************
   > File Name:   full_model_NA4_illegal_instruction_fetch_interrupt_enable_sup.sv
   > Description: 
   1. Configure the IOPMP Registers.
   2. Configure the interrupt suppress bit for an illegal execute access in the ENTRY_CFG Register.
   3. Congfigure the global interrupt enable.
   4. Send an instruction fetch transaction that must generate 0x03 error.
   ...
   ---------------------------------------------------------------
************************************************************************/
class full_model_NA4_illegal_instruction_fetch_interrupt_enable_sup extends full_model_base_test;
  `uvm_component_utils(full_model_NA4_illegal_instruction_fetch_interrupt_enable_sup)
  ...
  function new(string name = "full_model_NA4_illegal_instruction_fetch_interrupt_enable_sup", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
     ...
     // hwcfg2.prio_entry = 16
      ...
      entry_cfg.b_data   = 32'h90;                     // NA4 address mode with no permissions granted
      entry_cfg.index    = 3;
