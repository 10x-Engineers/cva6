/*************************************************************************
   > File Name:   full_model_src_no_r_cfg_x_disable.sv
   > Description: Send read address with prot b100 for Full MOdel with NA4 Addressing mode by disabling x permission in entry_cfg. Also not set SRCMD_R corresponding bit.
   1. Configure full address match and no execute permission.
   2. Corresponding SRCMD_R=0.
   3. Send an instruction fetch.
   ...
   ---------------------------------------------------------------
************************************************************************/
class full_model_src_no_r_cfg_x_disable extends full_model_base_test;
  `uvm_component_utils(full_model_src_no_r_cfg_x_disable)
  ...
  function new(string name = "full_model_src_no_r_cfg_x_disable", uvm_component parent = null);
  ...
  super.main_phase(phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      entry_cfg.b_data = 32'h10;                     // NA4 address mode with no (x specificly) permission granted
      entry_cfg.index = 3;
