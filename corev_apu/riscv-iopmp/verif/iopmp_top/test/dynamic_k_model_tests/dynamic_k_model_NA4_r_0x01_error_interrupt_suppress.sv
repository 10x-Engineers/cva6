class dynamic_k_model_NA4_r_0x01_error_interrupt_suppress extends dynamic_k_model_base_test;
  `uvm_component_utils(dynamic_k_model_NA4_r_0x01_error_interrupt_suppress)
  ...
  function new(string name = "dynamic_k_model_NA4_r_0x01_error_interrupt_suppress", uvm_component parent = null);
  ...
  task main_phase(uvm_phase phase);
    `uvm_info(get_name(), "MAIN PHASE STARTED", UVM_LOW);
      ...
      entry_cfg.wdata = 32'h130;                     // NAPOT address mode with ere suppression and no permissions granted
      entry_cfg.index = 9;
      entry_cfg.start(env.ahb_env.ahb_agnt.ahb_sqr);

      // The transaction is a success no error is reported
      #50ns;
      ar_seq.rrid      = 3;
      ar_seq.addr      = 364;
      ar_seq.id        = 3;
