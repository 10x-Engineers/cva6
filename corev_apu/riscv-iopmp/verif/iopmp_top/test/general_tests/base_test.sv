...
class base_test extends uvm_test;
    ...
    ////////
    int md_index, i, j;   //md_index for mdstall
...
`ifdef CFG_IOPMP_MDCFG_FMT_0
    ...
    if(SRCMD_FMT==0)begin
      ...
      $display("i____:%h",i);
      for(; i>=0; i--)begin
          if(md[i+1] == 1)begin //i+1 because srcmd 0th bit is for lock
            if(j>=0)begin
              mdcfg.index = i;
              mdcfg.b_data = j; 
...
`endif
    ...
    task ent_cfg(input int e_index = lwr_entry, bit [2:0] perm, bit [1:0] addr_mode, bit [5:0] suppress);
      if(e_index > 0)begin
        ...
        `uvm_info(get_name(), $sformatf("ent_cfg____b_Data perm=%h, addr_mode=%h, suppress=%h,  index = %h, e_index=%h", entry_cfg.b_data[2:0], entry_cfg.b_data[4:3], entry_cfg.b_data[10:5], entry_cfg.index, e_index), UVM_LOW);
      end
    endtask
