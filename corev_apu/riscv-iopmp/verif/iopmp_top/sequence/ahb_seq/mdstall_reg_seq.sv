...
class mdstall_reg_seq extends ahb_base_seq;
    `uvm_object_utils(mdstall_reg_seq)
        
    bit read;
    
    function new(string name = "mdstall_reg_seq");
        super.new(name);
    endfunction
    ...
    task body;
        ...
        if(read == 0)
        begin
            regmodel.mdstall.write(status,b_data); //Address 0x0030
            regmodel.mdstall.read(status,r_data); //Address 0x0030
        end  
        else if(read == 1)
        begin
            regmodel.mdstall.read(status,r_data); //Address 0x0030
