...
package iopmp_axi_pkg;
  ...
  typedef struct packed {
    logic [SLAVE_ID_WIDTH-1:0]  aw_id;       // 6 {MSI_Request,ID}
    logic [AXI_ADDR_WIDTH-1:0]  aw_addr;     // 64
    logic [7:0]                 aw_len;      // 8
    logic [2:0]                 aw_size;     // 3
    logic [1:0]                 aw_burst;
    logic                       aw_lock;
    ...
    logic [3:0]                 aw_qos;
    logic [3:0]                 aw_region;
    logic [SLAVE_USER_WDTH-1:0] aw_user;     // 11
  } slv_aw_channel_t;
  ...
  typedef struct packed {
    logic [SLAVE_ID_WIDTH-1:0]  b_id;        // 6
    logic [1:0]                 b_resp;      // 2
    logic [SLAVE_USER_WDTH-1:0] b_user;      // 11
  } slv_b_channel_t;
  ...
  typedef struct packed {
    logic [MASTER_ID_WIDTH-1:0]       id;
    logic [AXI_ADDR_WIDTH-1:0]        addr;
    logic [$clog2(MAX_BURST_LEN)-1:0] len;
    logic [1:0]                       size;
    logic                             burst;
    logic                             axlock;
    logic                             r;
    logic                             w;
    logic                             x;
    logic [10:0]                      rrid;     // 11 {WD_Tag,RRID}
  } transaction_t;
