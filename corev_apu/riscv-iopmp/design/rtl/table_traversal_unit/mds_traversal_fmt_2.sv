...
#(
  ...
  always_comb begin
    ...
    for (int cur_md = 1; cur_md  < (END_MD - START_MD); cur_md++) begin
      ...
      // Otherwise, a multiplier is used to get the exact range of associated entries
      if (is_k_power_of_2_l_i) begin
        lwr_entry_index = mdpresent_l_i[cur_md + START_MD] ? ((cur_md[7:0] + START_MD) << clog2_k_l_i) : '0;
        upr_entry_index = mdpresent_l_i[cur_md + START_MD] ? (((cur_md[7:0] + START_MD) + 1) << clog2_k_l_i) : '0;
      end
      else begin
        lwr_entry_index = mdpresent_l_i[cur_md + START_MD] ? ((cur_md[7:0] + START_MD) * k_l_i) : '0;
        upr_entry_index = mdpresent_l_i[cur_md + START_MD] ? (((cur_md[7:0] + START_MD) + 1) * k_l_i) : '0;
      end
