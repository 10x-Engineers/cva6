...
#(
  ...
  localparam logic [62:0] MD_NUM_MASK = ~('1 << int'(CFG.MD_NUM));  // Inidcates the implementation status of an MD (1 bit per MD) based on read-only field hwcfg0.md_num
  localparam logic [CFG.ENTRY_NUM-1:0] PRIO_ENTRY_MASK = ~('1 << int'(CFG.PRIO_ENTRY));   // Indicates the priority status of an entry (1 bit per entry) based on configured value
  ...
  if (CFG.PRIENT_PROG) begin : gen_programmable_prio_entry_vector
    ...
    if (CFG.ENTRY_NUM > 48) begin
      assign prio_entry_vec = {{(CFG.ENTRY_NUM-48){1'b0}}, prio_entry};
    end else begin
      assign prio_entry_vec = prio_entry[CFG.ENTRY_NUM-1:0];
    end
  end
  ...
  if (CFG.SRCMD_FMT_0) begin : drive_srcmd_en_enh
    for (genvar index = 0; index < int'(CFG.RRID_NUM); index = index+1) begin : gen_srcmd_en_enh
  ...
  if (CFG.MDCFG_FMT_0) begin : gen_mdcfg_fmt_0_write_path
    ...
    (
      ...
      .mdcfg_reg_swdata      (req_wdata[15:0]),                             // Write data for write request
  ...
  assign rfm_ttu = '{
    ...
    hwcfg1_rrid_num     : iopmp_reg.info_reg.hwcfg1.rrid_num[6:0],                  // Indicates the number of supported RRIDs
    ...
    mdcfg               : iopmp_reg.mdcfg_table[CFG.MD_NUM-1:0],                    // MDCFG TABLE registers when MDCFG Format is 0
    srcmd_table_0       : iopmp_reg.srcmd_table.srcmd_table_0[CFG.RRID_NUM-1:0],    // SRCMD TABLE registers struct when SRCMD Format is 0
    srcmd_table_2       : iopmp_reg.srcmd_table.srcmd_table_2[CFG.MD_NUM-1:0]       // SRCMD TABLE registers struct when SRCMD Format is 2
  };
  ...
  assign rfm_rap = '{
    entry_table     : iopmp_reg.entry_array[CFG.ENTRY_NUM-1:0],   // ENTRY ARRAY registers struct
    napot_size      : napot_size,                                 // Calculated value of NAPOT size per entry based on 64 bit entry address
