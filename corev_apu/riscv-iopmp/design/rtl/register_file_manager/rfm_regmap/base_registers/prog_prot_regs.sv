...
#(
  ...
  localparam MAX_COUNTER_VALUE = ((((int'(CFG.MD_NUM) + 20)/21) - 1) + ((int'(CFG.ENTRY_NUM) + 7)/8));
  ...
  logic [CFG.RRID_NUM-1:0] read_vec, rrid_stall_n;
  ...
  logic         is_normal_state_n, is_normal_state_q;  // Indicates the IOPMP is in the NORMAL state, if asserted
  logic         is_mdstall_swdata_zero, is_mdstallh_swdata_zero;    // Indicates if mdstall or mdstallh swdata is zero
  ...
  if (CFG.STALL_EN) begin : gen_stall_regs_write_path
    ...
    assign rridscp_rrid_swdata = (prog_prot_reg_swdata[15:0] <= {{9{1'b0}}, hwcfg1_rrid_num}) ?
                                 prog_prot_reg_swdata[15:0] :      // Write new value
                                 {{9{1'b0}}, hwcfg1_rrid_num};     // Configure to maximum supported value

    // Indicates if SW Write Data value on MDSTALL is zero
    assign is_mdstall_swdata_zero = !(|({mdstall_md_swdata, mdstall_exempt_swdata}));

    // Indicates if SW Write Data value on MDSTALLH is zero
    assign is_mdstallh_swdata_zero = !(|mdstallh_mdh_swdata);

    // A write on MDSTALL happens only when IOPMP is in NORMAL State and write data is zero when IOPMP is in STALL state
    // Otherwise the write on MDSTALL is ignored
    assign is_mdstall_write_valid = ((is_mdstall_swdata_zero && is_stall_state_q) || is_normal_state_q) && prog_prot_reg_write_valid;

    // A write on MDSTALLH happens only when IOPMP is in NORMAL State and write data is zero when IOPMP is in STALL state
    // Otherwise the write on MDSTALLH is ignored
    assign is_mdstallh_write_valid = ((is_mdstallh_swdata_zero && is_stall_state_q) || is_normal_state_q) && prog_prot_reg_write_valid;
    ...
    assign stall_by_md = {prog_prot_reg.mdstallh.mdh,mdstall_md_swdata};
    ...
    always_comb begin : stall_handling
      ...
      //  Case 1: When MDSTALL is written in NORMAL state
      //  Case 2: When RRIDSCP.op is written 1 in STALL state, i.e. stall the transaction ascociated with rridscp.rrid
      start_counter = (is_normal_state_q && mdstall_exempt_swen) || (rridscp_op_swen && rridscp_op_swdata[0]);

      // MDSTALL.is_busy is written 1 when write happens on MDSTALL in NORMAL state or RRIDSCP.op is written 1 in STALL state
      ...
      is_normal_state_n = is_normal_state_q;    // Indicates the IOPMP is in the NORMAL state, if asserted
      ...
      if (start_counter) begin
        ...
        is_normal_state_n = 1'b0;
      end

      // IOPMP has changed state to NORMAL again from STALL
      if (change_state[0] && is_stall_state_q) begin
        is_stall_state_n  = 1'b0;
        is_normal_state_n = 1'b1;
      end

      //****************************************************************************************************
      // Generate RRID STALL Signal
      //****************************************************************************************************
      rrid_stall_n = rrid_stall;

      if (mdstall_exempt_swen) begin
        ...
        if (CFG.SRCMD_FMT_0) begin : gen_srcmd_fmt_0_stall
          ...
          for (int rrid_stall_index = '0; rrid_stall_index < int'(CFG.RRID_NUM); rrid_stall_index++) begin : iterate_over_rrids
            ...
            rrid_stall_n[rrid_stall_index] = (mdstall_exempt_swdata ^ (|(srcmd_en_enh[rrid_stall_index] & stall_by_md)));
          end
        end
        ...
        if (CFG.SRCMD_FMT_1) begin : gen_srcmd_fmt_1_stall
          ...
          rrid_stall_n = ({CFG.MD_NUM{mdstall_exempt_swdata}} ^ stall_by_md);
        end
        ...
        if (CFG.SRCMD_FMT_2) begin : gen_srcmd_fmt_2_stall
          ...
          rrid_stall_n = {CFG.RRID_NUM{mdstall_exempt_swdata ^ (|stall_by_md)}};
        end
      end

      read_vec = 'b1 << rridscp_rrid_swdata;

      // Check if RRIDSCP.op is written 1 in STALL or HOLD mode.
      if (rridscp_op_swen && rridscp_op_swdata[0]) begin

        for (int i = 0; i < int'(CFG.RRID_NUM); i++) begin

          // If the bit corresponding to rrid s is 1, set the rrid_stall[s] to 1
          if (read_vec[i])
      ...
      if (rridscp_op_swen && rridscp_op_swdata[1]) begin

        for (int i = 0; i < int'(CFG.RRID_NUM); i++) begin

          // If the bit corresponding to rrid s is 1, set the rrid_stall_n[s] to 0
          if (read_vec[i])
      ...
      rridscp_stat_hwdata = {~rrid_stall[rridscp_rrid_swdata[$clog2(CFG.RRID_NUM)-1:0]], rrid_stall[rridscp_rrid_swdata[$clog2(CFG.RRID_NUM)-1:0]]};
      ...
      case (stall_counter_state_q)
        ...
        COUNT_DOWN: begin
          ...
          if (is_stall_counter_zero) begin
            ...
            stall_cntr_n          = MAX_COUNTER_VALUE[4:0];
          end
        end
      endcase
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin : stall_counter_flop
      if (!rst_n) begin
        ...
        stall_cntr_q          <= MAX_COUNTER_VALUE[4:0];
        is_stall_state_q      <= 1'b0;
        is_normal_state_q     <= 1'b1;
        rrid_stall            <= '0;
      end
      else begin
        ...
        is_normal_state_q     <= is_normal_state_n;
        rrid_stall            <= rrid_stall_n;
      end
    end
  end
