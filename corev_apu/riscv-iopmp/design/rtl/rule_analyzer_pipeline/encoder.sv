...
#(
  ...
  logic                             is_np_error;              // Set if any non-priority entry reports ERROR result
  ...
  assign is_np_error = |catch_np_error;
  ...
  always_comb begin
    ...
    if (operation_i == SEARCH) begin
      ...
      if (is_p_match) begin
        ...
        err_info_o = (CFG.ERROR_CAPTURE_EN && (!is_np_match) && is_np_error) ? {entry_err[4:1], is_supress} : err_info_i;
      end

      // Set index of selected entry for error reporting
      err_entry_index_o = (CFG.ERROR_CAPTURE_EN && (((!is_np_match) && is_np_error) || is_p_match)) ? {START_ENRTY[$clog2(CFG.ENTRY_NUM)-1:3], selected_index} : err_entry_index_i;
    end
    else begin
